...
using Ran.Dependencies.Geterators.Extensions;
...
public class ServiceRegistrationGenerator : IIncrementalGenerator
{
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        var methodName = context.AnalyzerConfigOptionsProvider.Select(
            static (c, _) =>
            {
                c.GlobalOptions.TryGetValue("build_property.InjectioName", out var methodName);
                c.GlobalOptions.TryGetValue(
                    "build_property.InjectioInternal",
                    out var methodInternal
                );
    ...
    private void ExecuteGeneration(
        ...
        (
            ImmutableArray<ServiceRegistrationContext?> Registrations,
            (string? AssemblyName, MethodOptions? MethodOptions) Options
        ) source
    )
    {
        var serviceRegistrations = source
            .Registrations.SelectMany(m =>
                m?.ServiceRegistrations ?? Array.Empty<ServiceRegistration>()
            )
        ...
        var moduleRegistrations = source
            .Registrations.SelectMany(m =>
                m?.ModuleRegistrations ?? Array.Empty<ModuleRegistration>()
            )
            .Where(m => m is not null)
            .ToArray();
        ...
        // add source file
        sourceContext.AddSource("AutoGegerators.g.cs", SourceText.From(result, Encoding.UTF8));
    }
    ...
    private static bool SyntacticPredicate(
    ...
    )
    {
        return (
            ...
            || (
                syntaxNode
                    is MemberDeclarationSyntax { AttributeLists.Count: > 0 } memberDeclaration
                && !memberDeclaration.Modifiers.Any(SyntaxKind.AbstractKeyword)
            );
    }

    private static ServiceRegistrationContext? SemanticTransform(
    ...
    private static ServiceRegistrationContext? SemanticTransformMethod(
        GeneratorSyntaxContext context
    )
    {
        ...
        var (diagnostics, hasServiceCollection, hasTagCollection) = ValidateMethod(
            methodDeclaration,
            methodSymbol
        );
        if (diagnostics.Any())
    ...
    private static ServiceRegistrationContext? SemanticTransformClass(
        GeneratorSyntaxContext context
    )
    {
    ...
    ) ValidateMethod(MethodDeclarationSyntax methodDeclaration, IMethodSymbol methodSymbol)
    {
    ...
    )
    {
        ...
        )
        {
            ...
            for (var index = 0; index < attributeClass.TypeParameters.Length; index++)
            {
                ...
                {
                    var service = typeArgument.ToDisplayString(
                        SymbolDisplayFormat.FullyQualifiedFormat
                    );
                ...
                {
                    implementationType = typeArgument.ToDisplayString(
                        SymbolDisplayFormat.FullyQualifiedFormat
                    );
                }
            }
        ...
        foreach (var parameter in attribute.NamedArguments)
        {
            ...
            switch (name)
            {
                ...
                case "Registration":
                    ...
                    break;
                case "Tags":
                    var tagsItems = value.ToString().Split(',', ';').Where(v => v.HasValue());
        ...
        bool includeInterfaces =
            registrationStrategy
                is KnownTypes.RegistrationStrategyImplementedInterfacesShortName
        ...
        if (includeInterfaces)
        {
            ...
            {
                var interfaceName = implementedInterface.ToDisplayString(
                    SymbolDisplayFormat.FullyQualifiedFormat
                );
        ...
        bool includeSelf =
            registrationStrategy
                is KnownTypes.RegistrationStrategySelfShortName
                    or KnownTypes.RegistrationStrategySelfWithInterfacesShortName;
    ...
    private static bool IsTransientAttribute(AttributeData attribute)
    {
        return attribute?.AttributeClass
            is {
                Name: KnownTypes.TransientAttributeShortName
                    or KnownTypes.TransientAttributeTypeName,
                ContainingNamespace:
                { Name: "Attributes", ContainingNamespace.Name: "Ran.Dependencies" }
            };
    }
    ...
    private static bool IsSingletonAttribute(AttributeData attribute)
    {
        return attribute?.AttributeClass
            is {
                Name: KnownTypes.SingletonAttributeShortName
                    or KnownTypes.SingletonAttributeTypeName,
                ContainingNamespace:
                { Name: "Attributes", ContainingNamespace.Name: "Ran.Dependencies" }
            };
    }
        ...
        return parameterSymbol?.Type
            is {
                ...
                ContainingNamespace:
                {
                    ...
                    ContainingNamespace:
                    { Name: "Extensions", ContainingNamespace.Name: "Microsoft" }
                }
            };
    }
